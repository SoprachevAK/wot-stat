...
class _OverrideLib(object):
    def __init__(self):
        ...
        self.overrideClassMethod = self.__hookDecorator(
            self.__overrideClassMethod)
        self.overrideStaticMethod = self.__hookDecorator(
            self.__overrideStaticMethod)
    ...
    def __registerEvent(self, handler, cls, method, debug=True, prepend=False):
        ...
        if hasattr(cls, evt):
            ...
            l = lambda *a, **k: self.__eventHandler(
                handler, debug, prepend, e, m, *a, **k)
            l.__name__ = method
    ...
    def __overrideMethod(self, handler, cls, method, debug=True):
        orig = getattr(cls, method)
        new_method = lambda *a, **k: self.__overrideHandler(
            handler, orig, debug, *a, **k)
    ...
    def __overrideStaticMethod(self, handler, cls, method, debug=True):
        ...
        new_method = staticmethod(
            lambda *a, **k: self.__overrideHandler(handler, orig, debug, *a, **k))
        self.__override(cls, method, new_method)
